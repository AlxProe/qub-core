        ...
        match mode {
            QubJinMode::Infuse => {
                if self.qub_jin_dialog.amount.trim().is_empty() || self.qub_jin_dialog.amount.trim() == "0" {
                    // HF119/v1.7.8: never load/scan the chain on the UI thread just
...
fn hf119_pre_qub_jin_action_sync(settings: &Settings) {
    if !settings.p2p.enabled { return; }
    // HF119/v1.7.8: QUB/JIN Infuse/Melt needs a fresh local ledger, but the GUI
